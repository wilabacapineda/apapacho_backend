...
const customIcons = document.getElementById('customIcons')
const carroTienda_div = document.getElementById("carroDiv")
const carroTienda_title = carroTienda_div.querySelector("h3 #close")
const iconMenuTienda = document.querySelectorAll(".iconMenuTienda")

/*
buscaTienda_icon.addEventListener("click", () => {
    if(buscaTienda_input.style.width){
        iconMenuTienda.forEach( a => {            
            a.style.flexDirection="column"   
            document.getElementById(`titleMenuTienda_${a.getAttribute('data-target')}`).innerHTML= ''
        })
        buscaTienda_input.style.width=""
        buscaTienda_input.style.padding="0"
        buscaTienda_resultado.style.display="none"
        customIcons.style.flexDirection='column'
        customIcons.style.alignItems="center"
    } else {
        buscaTienda_input.style.width="300px"
        buscaTienda_input.style.padding="1rem"
        buscaTienda_resultado.style.display="flex"
        customIcons.style.flexDirection='row'
        customIcons.style.alignItems="baseline"
        iconMenuTienda.forEach( a => {            
            a.style.flexDirection="row"        
            document.getElementById(`titleMenuTienda_${a.getAttribute('data-target')}`).innerHTML= a.title
        })
    }    
})
